...
void set_cpu_output(void)
{
    ...
    for (i = 0; i < LENGTH_OF(curr_load); i++) {
    ...
    idle_time_delta = idle_time_delta < 0 ? -idle_time_delta :
                                             idle_time_delta;
...
/*
 * The Desired wireless interface data exists on the 3rd line
 * of /proc/net/wireless's output. If the file descriptor is null
 * it's likely because there is no active connection. As a result,
 * we'd like to output nothing. The first '.' in the output indicates
 * the end of the link quality value which is what we want.
 */
